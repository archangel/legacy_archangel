...
module Archangel
  RSpec.describe GlyphiconHelper, type: :helper do
    context "#glyphicon_icon" do
      ...
      it "builds icon" do
        expect(helper.glyphicon_icon("user"))
          .to eq(
      ...
      it "builds icon with text" do
        expect(helper.glyphicon_icon(:user, text: "User"))
          .to eq(
            "<span class=\"glyphicon glyphicon-user\" " \
            "aria-hidden=\"true\"></span> " \
            "User"
      ...
      it "builds icon with additional classes" do
        expect(helper.glyphicon_icon(:user, class: "foo"))
          .to eq(
      ...
      it "builds icon with other attributes" do
        expect(helper.glyphicon_icon(:user, data: { id: 123 }))
          .to eq(
            "<span data-id=\"123\" " \
